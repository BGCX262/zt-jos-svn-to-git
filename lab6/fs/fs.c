...
alloc_block_num(void)
{
	...
	for (num_high = super->s_nblocks >> 5; num_high >= 0;num_high--) {
		if (bitmap[num_high] != 0x0)			//check if all the block is allocated
			for (num_low = 0; num_low < 32; num_low++)
				if (bitmap[num_high] & (1 << num_low)) {
					bitmap[num_high] &= ~(1 << num_low);
					write_block(2+((num_high<<5) | num_low)/BLKBITSIZE);
					return ((num_high<<5) | num_low);
				}
	}
	return -E_NO_DISK;
}
...
read_bitmap(void)
{
	...
	for (i = (super->s_nblocks+BLKBITSIZE-1)/BLKBITSIZE+2; i > 1 ; i--)
		if ((r=read_block(i, &blk)) < 0)
			panic("Read bitmap error: %08x", r);
	
	bitmap = (uint32_t *) blk;				// save the block no 2 blk to bitmap
...
file_truncate_blocks(struct File *f, off_t newsize)
{
	...
	//assert(newsize % BLKSIZE == 0);
	old_nblocks = (f->f_size+BLKSIZE-1)/BLKSIZE;
	new_nblocks = (newsize+BLKSIZE-1)/BLKSIZE;

	for (bno = new_nblocks; bno < old_nblocks; bno++) {
		if ((r=file_clear_block(f, bno)) < 0)
			panic("file truncate error: %08x", r);
	}
	if (new_nblocks < old_nblocks && 
			new_nblocks < NDIRECT && f->f_indirect != 0) {
...
file_flush(struct File *f)
{
	...
	//assert(f->f_size%BLKSIZE == 0);
	nblocks = (f->f_size+BLKSIZE-1) / BLKSIZE;
	for (i = 0; i < nblocks; i++) {
		if ((r=file_map_block(f, i, &dbno, 0)) < 0)
			panic("file_flush error:%e", r);
		if (block_is_dirty(dbno)) {
			write_block(dbno);
		}
	}
}
