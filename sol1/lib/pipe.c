...
_pipeisclosed(struct Fd *fd, struct Pipe *p)
{
	...
	// the pipe is closed.
    // return pageref((void *)fd) == pageref((void *)p);
    u_long env_runs1, env_runs2;
    u_long reffd, refp;
    while(1) {
        env_runs1 = env->env_runs;
        reffd = pageref((void *)fd);
        refp = pageref((void *)p);
        env_runs2 = env->env_runs;
        if(env_runs1 == env_runs2) {
            return reffd == refp;
        }
        else {
            if(reffd == refp)
...
piperead(struct Fd *fd, void *vbuf, u_int n, u_int offset)
{
    ...
    for(i = 0; i < n; i++) {
        ...
        *((u_char *)vbuf + i) = p->p_buf[p->p_rpos%BY2PIPE];
        // printf("[%08x] piperead: p->p_rpos = %d p->p_wpos = %d char %c\n", sys_getenvid(), p->p_rpos, p->p_wpos, *((u_char *)vbuf + i));
        p->p_rpos++;
    }
    return (int)n;
...
pipewrite(struct Fd *fd, const void *vbuf, u_int n, u_int offset)
{
    ...
    for(i = 0; i < n; i++) {
        ...
        p->p_buf[p->p_wpos%BY2PIPE] = *((u_char *)vbuf + i);
        p->p_wpos++;
    }
    return (int)n;
